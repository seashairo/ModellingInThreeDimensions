...
ParallaxShaderClass::ParallaxShaderClass(){
}

ParallaxShaderClass::ParallaxShaderClass(const ParallaxShaderClass& other){
}
...
void ParallaxShaderClass::ShutdownShader(){
	m_effects = 0;
...
// Shutsdown the shader
void ParallaxShaderClass::Shutdown(){
	ShutdownShader();
	return;
}

void ParallaxShaderClass::Initialize(ID3D10Device* device){
	HRESULT result;
	...
	result = D3DX10CreateEffectFromFile(L"../ModellingInThreeDimensions/data/Parallax_Mapping.fx", NULL, NULL, "fx_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, 
	...
	if(FAILED(result))
	{
		if(errorMessage)
		{
			Log::COut("parallax_normal_mapping2.fx has failed to compile");
	...
	if(!m_techniques)
	{
		Log::COut("ParallaxShader does not contain this technique");
	}
	...
	if(FAILED(result))
	{
		Log::COut("ParallaxShader passed-in parameters are not sufficient");
	}
	...
	m_parallaxWorldInverseTranspose = m_effects->GetVariableByName("worldInverseTransposeMatrix")->AsMatrix();
    m_parallaxWorldViewProjection = m_effects->GetVariableByName("worldViewProjectionMatrix")->AsMatrix();
	//m_parallaxWorldViewProjection = m_effects->GetVariableByName("viewProjectionMatrix")->AsMatrix();
	...
	m_scalebias = m_effects->GetVariableByName("scalebias")->AsVector();
...
//Render generally
void ParallaxShaderClass::Render(ID3D10Device* device, int indexCount, int vertexCount, int instanceCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, D3DXMATRIX worldInverseTranspose, D3DXMATRIX worldViewProjection, 
							  ID3D10ShaderResourceView** textureArray, D3DXVECTOR3 lightDirection, D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor, D3DXVECTOR3 cameraPosition,
								D3DXVECTOR4 specularColor, float specularPower, D3DXVECTOR2 scaleBias,//Shadow
								D3DXMATRIX lightViewMatrix, D3DXMATRIX lightProjectionMatrix,ID3D10ShaderResourceView* depthMapTexture,
								D3DXVECTOR3 lightPosition){
									
	m_scalebias->SetFloatVector((float*) &scaleBias);
	m_worldMatrixPtr->SetMatrix((float*)&worldMatrix);
	m_viewMatrixPtr->SetMatrix((float*)&viewMatrix);
    m_projectionMatrixPtr->SetMatrix((float*)&projectionMatrix);
	m_parallaxWorldInverseTranspose->SetMatrix((float*)&worldInverseTranspose);
	m_parallaxWorldViewProjection->SetMatrix((float*)&worldViewProjection);
	...
	m_texturePtr->SetResourceArray(textureArray, 0, 3);
